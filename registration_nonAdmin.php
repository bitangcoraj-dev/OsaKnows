...
if (isset($_POST['submit'])) {
    ...
    $idpictureDestination = 'idPic/' . basename($idpictureName); //create a folder named 'idPic' so that images uploaded would be stored inside the folder

    if ($password !== $confirmPassword) {
        $message = "Passwords do not match.";
    } else {
        // Check for duplicate email or ID number
        $checkDuplicate = $conn->prepare("SELECT * FROM register_students WHERE adzu_email = ? OR id_number = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            if (move_uploaded_file($idpictureTmpName, $idpictureDestination)) {
                ...
                $sql = $conn->prepare("INSERT INTO register_students (u_fullname, u_course, u_year, id_number, adzu_email, u_password, id_picture) 
                                       VALUES (?, ?, ?, ?, ?, ?, ?)");

                //bind parameters 
                $sql->bind_param("sssssss", $fullname, $course, $year, $idnumber, $email, $hashedPassword, $idpictureName);
...
<body>
    <div class = "registration-container"> 
        <h2> Create a Student/Staff Account </h2> 
    ...
    <form method="POST" action="" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select id="course" name="course" required>
                ...
                <option value="NA">Not Applicable (For Staffs and Faculty)</option>
                <option value="BSA">BS Accountancy</option>
                <option value="BSBA">BS Business Administration</option>
                <option value="BSLM">BS Legal Management</option>
                <option value="BSMA">BS Management Accounting</option>
                <option value="BSOA">BS Office Administration</option>

                <option value="BAC">BA Communication</option>
                <option value="BAELS">BA English Language Studies</option>
                <option value="BAIS">BA International Studies</option>
                <option value="BAP">BA Philosophy</option>
                <option value="BSP">BS Psychology</option>

                <option value="BSB">BS Biology</option>
                <option value="BSBE">BS Biomedical Engineering</option>
                ...
                <option value="BSNMCA">BS New Media and Computer Animation</option>
                <option value="AEET">Associate in Electronics Engineering Technology</option>
                ...
                <option value="CPE">Certificate in Professional Education</option>
